...
exports.getCreatorByEmail = async (req, res) => {
  try {
    ...
    // Analyze if Impact IDs are real or auto-generated
    const impactIdAnalysis = {
      hasImpactId: !!creator.impactId,
      hasImpactSubId: !!creator.impactSubId,
      impactIdValue: creator.impactId,
      impactSubIdValue: creator.impactSubId,
      
      // Check if IDs look auto-generated vs real
      impactIdLooksReal: creator.impactId ? !creator.impactId.startsWith('impact_') && !creator.impactId.startsWith('auto_') : false,
      impactSubIdLooksReal: creator.impactSubId ? !creator.impactSubId.includes('_') || creator.impactSubId.length > 15 : false,
      
      // Auto-generated patterns to identify
      isAutoGenerated: {
        impactId: creator.impactId ? (creator.impactId.startsWith('impact_') || creator.impactId.startsWith('auto_')) : false,
        impactSubId: creator.impactSubId ? (creator.impactSubId.includes('_') && creator.impactSubId.length < 20) : false
      }
    };

    console.log('🔍 Impact ID Analysis:', impactIdAnalysis);

    // Try to verify with real Impact.com API if Sub ID exists
    let realImpactVerification = null;
    if (creator.impactSubId) {
      try {
        ...
        realImpactVerification = {
          checkedWithImpactAPI: true,
          existsInImpact: !!foundInImpact,
          impactData: foundInImpact || null
        };
        
        console.log('✅ Impact API verification result:', realImpactVerification);
      } catch (impactError) {
        console.log('⚠️ Could not verify with Impact API:', impactError.message);
        realImpactVerification = {
          checkedWithImpactAPI: false,
    ...
    res.status(200).json({
      ...
      analysis: {
        ...
        // NEW: Real vs Auto-generated analysis
        impactIdType: impactIdAnalysis.impactIdLooksReal ? 'REAL' : (creator.impactId ? 'AUTO_GENERATED' : 'MISSING'),
        impactSubIdType: impactIdAnalysis.impactSubIdLooksReal ? 'REAL' : (creator.impactSubId ? 'AUTO_GENERATED' : 'MISSING'),
        hasRealImpactIds: impactIdAnalysis.impactIdLooksReal && impactIdAnalysis.impactSubIdLooksReal,
        impactIdDetails: impactIdAnalysis,
        
        // NEW: Real Impact.com API verification
        impactAPIVerification: realImpactVerification,
        isVerifiedInImpact: realImpactVerification?.existsInImpact || false
      }
    });
  } catch (error) {
